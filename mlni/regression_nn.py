...
# class RB_RepeatedHoldOut_NN_Regression(WorkFlow):
#     """
...
#         self._algorithm = NNRegressionAlgorithm(x_tensor, y_tensor, self._output_dir, df_header, self._n_iterations,
...
# class NNRegressionAlgorithm(RegressionAlgorithm):
#     '''
#     NN regression.
#     '''
...
#         # train the NN
#         train_network(model, self._output_dir, fi, x_train, y_train, x_test, y_test, 
#                     self._epochs, self._batch_size, init_state, self._df_header, 
#                     device == torch.device("mps"), self._lr, self._weight_decay, self._optimizer)
...
import copy
from sklearn.model_selection import ShuffleSplit
from mlni.utils import time_bar, neural_network_regression_3LinerLayers, train_network
...
class RB_RepeatedHoldOut_NN_Regression(WorkFlow):
    ...
    def run(self):
        ...
        self._algorithm = NNRegressionAlgorithm(x, y, self._output_dir, df_header, self._n_iterations,
                                                batch_size=self._batch_size,
                                                epochs=self._epochs,
                                                lr=self._lr,
        ...
        self._validation = RepeatedHoldOut(self._algorithm, input_dim, 
...
class NNRegressionAlgorithm(RegressionAlgorithm):
    def __init__(self, x, y, output_dir, df_header, n_iterations=100, batch_size=64, 
                 epochs=10, lr=0.0001, weight_decay=1e-4, optimizer='Adam', gpu=True, verbose=False):
        self._device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
        
        # Move tensors to the correct device if they aren't already
        self._x = x.to(self._device) if not isinstance(x, torch.Tensor) else x
        self._y = y.to(self._device) if not isinstance(y, torch.Tensor) else y
    ...
    def _lauch_nn(self, input_dim, x_train, x_test, y_train, y_test, fi):
        # Ensure all inputs are on the correct device and are tensors
        x_train = x_train.to(self._device) if not isinstance(x_train, torch.Tensor) else x_train
        y_train = y_train.to(self._device) if not isinstance(y_train, torch.Tensor) else y_train
        x_test = x_test.to(self._device) if not isinstance(x_test, torch.Tensor) else x_test
        y_test = y_test.to(self._device) if not isinstance(y_test, torch.Tensor) else y_test
        ...
        train_network(model, self._output_dir, fi, x_train, y_train, x_test, y_test, 
                     ...
                     self._lr, self._weight_decay, self._optimizer)

    def evaluate(self, input_dim, train_index, test_index, fi):
        # Convert indices to tensor and move to the correct device
        train_index = torch.tensor(train_index, dtype=torch.long, device=self._device)
        test_index = torch.tensor(test_index, dtype=torch.long, device=self._device)
