from mlni.classification_nn import RB_RepeatedHoldOut_NN_Classification
...
def classification_roi(feature_tsv, output_dir, cv_repetition, cv_strategy='hold_out', batch_size=64, epochs=1500, lr=0.0001, weight_decay=1e-4, optimizer='Adam', n_threads=8, gpu=True, seed=None, verbose=False):
    """
    Core function for classification with ROI-based features using Lasso classification
    Args:
    ...
    """
    print('MLNI for a classification with nested CV...')
    ...
    print('Starts classification with NN...')
    ...
    if cv_strategy == 'hold_out':
        wf_classification = RB_RepeatedHoldOut_NN_Classification(input_data, split_index, os.path.join(output_dir, 'classification'),
