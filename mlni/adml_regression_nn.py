...
def regression_roi(feature_tsv, output_dir, cv_repetition, cv_strategy='hold_out', 
                   batch_size=64, epochs=1500, lr=0.0001, weight_decay=1e-4, 
                   optimizer='Adam', n_threads=8, gpu=True, seed=None, verbose=False):
    """
    ...
    # Data split
    print(f'Data split was performed based on validation strategy: {cv_strategy}...\n')
    
    if os.path.isfile(os.path.join(output_dir, f'data_split_stratified_{cv_repetition}-holdout.pkl')):
        split_index = pickle.load(open(os.path.join(output_dir, f'data_split_stratified_{cv_repetition}-holdout.pkl'), 'rb'))
    else:
        split_index, _ = make_cv_partition(input_data.get_y(), cv_strategy, output_dir, cv_repetition, seed=seed)
    print('Data split has been done!\n')
    ...
    if cv_strategy == 'hold_out':
        x = torch.tensor(input_data.get_x(), dtype=torch.float32, device=device)
        y = torch.tensor(input_data.get_y_raw(), dtype=torch.float32, device=device)

        class DeviceInput:
            def __init__(self, x, y, original_input, device):
                self._x = x
                self._y = y
            ...
            def get_participant_session_id(self):
                return self._original_input.get_participant_session_id()

        device_input = DeviceInput(x, y, input_data, device)

        wf_regression = RB_RepeatedHoldOut_NN_Regression(
            input=device_input, 
